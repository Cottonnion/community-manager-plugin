...
class SubscriptionsAdmin {
    ...
    public function init() {
        ...
        add_action( 'wp_ajax_update_subscription', array( $this, 'ajax_update_subscription' ) );
        add_action( 'wp_ajax_delete_subscription', array( $this, 'ajax_delete_subscription' ) );
        add_action( 'wp_ajax_save_subscription', array( $this, 'ajax_save_subscription' ) );
    }
    ...
    public function ajax_get_subscription_details() {
        ...
        if ( ! check_ajax_referer( 'labgenz_admin_subscriptions_nonce', 'nonce', false ) ) {
            wp_send_json_error( array( 'message' => 'Invalid security token sent.' ) );
        }
    ...
    public function ajax_update_subscription() {
        ...
        $this->log_subscription_change( $user_id, 'update', array(
            'status'       => $status,
            'expiry_date'  => $expiry_date,
            'auto_renewal' => $auto_renewal,
        ) );

        wp_send_json_success( array( 'message' => 'Subscription updated successfully.' ) );
    }
    ...
    public function ajax_delete_subscription() {
        ...
        $this->log_subscription_change( $user_id, 'delete', array(
        ...
        wp_send_json_success( array( 'message' => 'Subscription deleted successfully.' ) );
    }
    ...
    public function ajax_save_subscription() {
        ...
        if ( empty( $plan_name ) ) {
            wp_send_json_error( array( 'message' => 'Subscription plan not found.' ) );
        }
        ...
        $this->log_subscription_change( $user_id, 'create', array(
            'subscription_id' => $subscription_id,
            'plan_id'         => $plan_id,
            'plan_name'       => $plan_name,
            'status'          => $status,
        ...
        wp_send_json_success( array( 'message' => 'Subscription created successfully.' ) );
    }
    ...
    private function log_subscription_change( $user_id, $action, $data ) {
        ...
        $logs = get_option( 'labgenz_subscription_logs', array() );
