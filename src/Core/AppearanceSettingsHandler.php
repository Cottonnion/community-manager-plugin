...
class AppearanceSettingsHandler {
	...
	public static function init() {
		add_action( 'wp_head', array( __CLASS__, 'add_google_fonts' ) );
		add_action( 'admin_head', array( __CLASS__, 'add_google_fonts' ) );
	}
	...
	public static function save_appearance_settings_ajax() {
		...
		try {
			$ajax->handle_request( array( __CLASS__, 'process_save_appearance_settings' ), 'labgenz_appearance_nonce' );
		} catch ( \Throwable $e ) {
			wp_send_json_error(
				array(
