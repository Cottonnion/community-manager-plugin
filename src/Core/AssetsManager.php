...
class AssetsManager {
	 ...
	 */	public function __construct() {
		...
		add_action('wp_enqueue_scripts', function() {
				wp_enqueue_script('xlsx-js', 'https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js', array(), '0.18.5', true);
		}, 20);
		...
		// Enqueue map assets
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_map_assets' ), 30 );
	}
	...
	public function enqueue_map_assets(): void {
		...
		if ( bp_is_group() && bp_is_current_action( 'members-map' ) ) {
			...
			wp_enqueue_style(
				...
				array('leaflet-markercluster-default-css'),
				'7.0.4'
			);
			...
			wp_enqueue_script( 
				'members-map-js', 
				LABGENZ_CM_URL . 'public/assets/js/members-map.js', 
				array( 'jquery', 'leaflet-js', 'leaflet-markercluster-js' ), 
				'7.0.4', 
				true 
			);
