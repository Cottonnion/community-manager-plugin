...
class InviteHandler {
    ...
    public function __construct() {
        ...
        $this->ajax_handler->register_ajax_actions([
            'labgenz_invite_user' => [$this, 'handle_invite_user'],
            'labgenz_get_invited_members' => [$this, 'get_invited_members'],
            'labgenz_get_profile_types' => [$this, 'get_profile_types'],
        ]);
    }
     ...
     * @since 1.0.0
     */
    public function handle_invite_user($request = null) {
        $this->ajax_handler->handle_request(function($data) {
     ...
     * @since 1.0.0
     */
    public function get_invited_members($request = null) {
        $this->ajax_handler->handle_request(function($data) {
            $current_user_id = get_current_user_id();
            if (!$current_user_id) {
                return new \WP_Error('not_logged_in', 'User not logged in');
     ...
     * @since 1.0.0
     */
    public function get_profile_types($request = null) {
        $this->ajax_handler->handle_request(function($data) {
            if (function_exists('bp_get_member_types')) {
                ...
                $profile_types = ['member', 'moderator', 'organizer'];
            }
            return ['profile_types' => array_values($profile_types)];
        }, 'inst3d_group_management_nonce');
    }
}
