	...
	<?php if ( ! empty( $data['members']['members'] ) || ! empty( $data['invited_users'] ) ) : ?>
		...
		<table class="members-table" id="members-table">
			...
			foreach ( $data['members']['members'] as $member ) :
				...
				?>
				<tr data-user-id="<?php echo esc_attr( $member->ID ); ?>" class="member-row" style="transition: background-color 0.2s ease; border-bottom: 1px solid #f0f0f0;">
			...
			foreach ( $data['invited_users'] as $user_id => $invitation ) :
				...
				?>
				<tr data-user-id="<?php echo esc_attr( $user_id ); ?>" class="invited-row">
